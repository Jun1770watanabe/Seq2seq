...
def load_data_from_path(vocabulary, path):
...
def load_data_using_dataset_api(
    ...
    def _transform_line(vocabulary, line):
        words = line.strip().split()
        return numpy.array([vocabulary.get(w, UNK) for w in words], numpy.int32)
...
def main():
    ...
    parser.add_argument('--save', '-s', type=str, help='save a snapshot of the training')
    ...
    if args.use_dataset_api:
        ...
        train_data = load_data_using_dataset_api(
            ...
            target_ids, args.TARGET, _filter_func,
        )
    else:
        # Load all records on memory.
        train_source = load_data_from_path(source_ids, args.SOURCE)
        train_target = load_data_from_path(target_ids, args.TARGET)
    ...
    trainer.extend(extensions.PrintReport(['epoch', 'iteration', 'main/loss',
        'main/perp', 'validation/main/bleu', 'elapsed_time']),
        trigger=(args.log_interval, 'iteration'))
    trainer.extend(extensions.PlotReport(['main/loss'], 
        x_key='iteration', file_name='loss.png', trigger=(200, "iteration")))
    trainer.extend(extensions.PlotReport(['validation/main/bleu'],
        x_key='iteration', file_name='bleu.png', trigger=(args.validation_interval, 'iteration')))
    ...
    if args.validation_source and args.validation_target:
        test_source = load_data_from_path(source_ids, args.validation_source)
        test_target = load_data_from_path(target_ids, args.validation_target)
        ...
        # test_iter = iterators.SerialIterator(test_data, args.batchsize, repeat=False, shuffle=False)
        # trainer.extend(extensions.Evaluator(test_iter, model, device=0))
        trainer.extend(translate, trigger=(args.validation_interval, 'iteration'))
        trainer.extend(CalculateBleu(model, test_data, 'validation/main/bleu', device),
            trigger=(args.validation_interval, 'iteration'))
